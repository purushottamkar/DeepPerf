    ...
    def convert(srcfile, targetfile, alignfile, batchsize, seqlength, outfile, num_sents,
        ...
        for _, (src_orig, targ_orig) in \
            ...
            if chars == 1:
                ...
                src_orig = src_indexer.clean(src_orig.strip())
                targ_orig = target_indexer.clean(targ_orig.strip())
            targ = [target_indexer.BOS] + targ_orig.strip().split() + [target_indexer.EOS]#targ_orig.strip().split() #
            ...
            if len(targ) > newseqlength1 or len(src) > newseqlength or len(targ) < 3 or len(src) < 3:
                dropped += 1
                continue
            targ = pad(targ, newseqlength1+1, target_indexer.PAD)
            targ_char = []
            for word in targ:
                if chars == 1:
                    word = char_indexer.clean(word)
