...
def get_data(args):
    ...
    target_indexer = Indexer1()#Indexer(["<blank>","<unk>","<s>","</s>"])
    ...
    def convert(srcfile, targetfile, alignfile, batchsize, seqlength, outfile, num_sents,
        ...
        targets = np.zeros((num_sents, newseqlength1), dtype=int)
        target_output = np.zeros((num_sents, newseqlength1), dtype=int)
        ...
        for _, (src_orig, targ_orig) in \
            ...
            if chars == 1:
                ...
                src_orig = src_indexer.clean(src_orig.strip())
                targ_orig = target_indexer.clean(targ_orig.strip())
            targ = targ_orig.strip().split()# [target_indexer.BOS] + targ_orig.strip().split() + [target_indexer.EOS]
            ...
            if len(targ) > newseqlength1 or len(src) > newseqlength or len(targ) < 1 or len(src) < 3:#or len(targ) < 3
                dropped += 1
                continue
            targ = targ#pad(targ, newseqlength+1, target_indexer.PAD)
            targ_char = []
            for word in targ:
                if chars == 1:
                    word = char_indexer.clean(word)
            ...
            targets[sent_id] = targ#np.array(targ[:-1],dtype=int)
            target_lengths[sent_id] = (targets[sent_id] != 1).sum()
            if chars == 1:
                targets_char[sent_id] = np.array(targ_char[:-1], dtype=int)
            target_output[sent_id] = targ#np.array(targ[1:],dtype=int)
...
def main(arguments):
    ...
    parser.add_argument('--maxwordlength', help="For the character models, words are "
                                           "(if longer than maxwordlength) or zero-padded "
                                            "(if shorter) to maxwordlength", type=int, default=20)
